...
const AircraftMarker: React.FC<AircraftMarkerProps> = ({ map, flights, onFlightSelect }) => {
  ...
  // SVG Template für bessere Performance - das SVG zeigt standardmäßig nach Norden (0°)
  const createSvgElement = useCallback((flight: Flight): SVGSVGElement => {
    const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
    
    // SVG Grundkonfiguration
    svg.setAttribute('width', '28');
    svg.setAttribute('height', '28');
    svg.setAttribute('viewBox', '0 0 512 512');
    svg.setAttribute('version', '1.1');
    
    // Erstelle die Flugzeug-Path
  ...
  // Optimized marker creation function mit SVG
  ...
  // SVG Rotation Update - direktes SVG transform verwenden
  const updateMarkerAppearance = useCallback((
    ...
    // SVG zeigt standardmäßig nach Norden (0°) - direkter Heading-Wert verwenden
    const rotationAngle = normalizedHeading;
    const scaleValue = isSelected ? 1.2 : 1.0;
    
    console.log(`🧭 SVG Flight ${flight.flightId}: Raw=${rawHeading}°, Normalized=${normalizedHeading}°, SVG Transform=${rotationAngle}°`);
    
    // Finde das SVG-Element und setze SVG-Transform
    const svgElement = element.querySelector('svg') as SVGSVGElement;
    if (svgElement) {
      // SVG Transform verwenden statt CSS Transform für präzisere Rotation
      svgElement.setAttribute('transform', `rotate(${rotationAngle}) scale(${scaleValue})`);
      svgElement.style.filter = filter;
      
      // Transform-Origin für SVG setzen
      svgElement.style.transformOrigin = 'center center';
      
      console.log(`✅ SVG Flight ${flight.flightId} SVG Transform: rotate(${rotationAngle}) scale(${scaleValue})`);
      console.log(`🧭 SVG Direction: ${getDirectionName(normalizedHeading)}`);
    }
  ...
  const createClickHandler = useCallback((flight: Flight) => {
    return (e: MouseEvent) => {
      e.stopPropagation();
      console.log(`🎯 SVG Flight clicked: ${flight.flightId}`);
      ...
      if (selectedMarkerIdRef.current && markerElementsRef.current[selectedMarkerIdRef.current]) {
        ...
        if (prevFlight) {
          console.log(`🔄 Removing SVG highlight from ${selectedMarkerIdRef.current}`);
      ...
      console.log(`✅ SVG Selected: ${flight.flightId}`);
  ...
  useEffect(() => {
    console.log(`🛩️ SVG AircraftMarker effect - ${flights.length} flights, map loaded: ${map?.loaded()}`);
    
    if (!map || !map.loaded()) {
      console.log("🔄 Map not loaded, waiting...");
      const onMapLoad = () => {
        console.log("✅ Map loaded, updating SVG markers");
        updateMarkers();
    ...
    function updateMarkers() {
      console.log(`🔄 Updating ${flights.length} SVG flight markers`);
      ...
      existingMarkerIds.forEach(flightId => {
        if (!currentFlightIds.has(flightId)) {
          console.log(`🗑️ Removing SVG marker for departed flight ${flightId}`);
      ...
      requestAnimationFrame(() => {
        ...
        const processBatch = () => {
          ...
          if (end < flights.length) {
            ...
            console.log(`✅ SVG Markers updated - Updated: ${updatedCount}, Created: ${createdCount}, Total: ${Object.keys(markersRef.current).length}`);
          }
        };
  ...
  useEffect(() => {
    return () => {
      console.log(`🧹 Cleaning up ${Object.keys(markersRef.current).length} SVG markers`);
